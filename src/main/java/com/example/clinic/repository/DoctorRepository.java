...
public interface DoctorRepository extends JpaRepository<Doctor, Integer> {
    ...
    @Query("SELECT d FROM Doctor d JOIN d.speciality s JOIN d.comments c " +
            "WHERE s.id = :specialityId " +
            "GROUP BY d " +
            "HAVING COUNT(c) >= 5 AND AVG(c.pointEvaluation) > 3 " +  // Ensure at least 10 comments and average pointEvaluation above 3
            "ORDER BY AVG(c.pointEvaluation) DESC")
    List<Doctor> findTop5DoctorsBySpecialityOrderByPointEvaluationDesc(Integer specialityId, Pageable pageable);
}
